    ...
    if (!existingUser) {
      ...
      if (account?.suspended) {
        ...
        targetUserId = existingUser._id;
             ...
             // Step 3.5: Check for duplicate invites (for adults only)
             if (inviteType === 'adult' && cliqId) {
               console.log(`[INVITE_CREATE] Checking for duplicate adult invite to: ${emailNorm}, cliqId: ${cliqId}`);
               
               const existingInvites = await convexHttp.query(api.invites.getInvitesByEmail, {
                 email: emailNorm,
               });
               
               // Filter for pending invites to the same cliq
               const duplicateInvite = existingInvites.find((inv: any) => 
                 inv.status === 'pending' && 
                 !inv.used && 
                 inv.cliqId?.toString() === cliqId &&
                 inv.inviteType === 'adult'
               );
